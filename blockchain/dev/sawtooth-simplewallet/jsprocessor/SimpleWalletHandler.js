...
const makeDeposit =
  (context, address, amount, user) => (possibleAddressValues) => {
    ...
    if (stateValueRep == null || stateValueRep == "") {
      ...
      if (balance < amount) {
        throw new InvalidTransaction(
          `Not enough money. The amount should be lesser or equal to ${balance}`
        );
      }
...
const makeTransfer =
  (context, senderAddress, amount, receiverAddress) =>
  (possibleAddressValues) => {
    ...
    if (senderBalance < amount) {
      throw new InvalidTransaction(
        "Not enough money to perform transfer operation"
      );
    } else {
    ...
    return _decodeRequest(transactionProcessRequest.payload)
      ...
      .then((update) => {
        ...
        if (typeof amount !== "number" || amount <= MIN_VALUE) {
          throw new InvalidTransaction(
            `Value must be an integer ` + `no less than 1`
          );
        }

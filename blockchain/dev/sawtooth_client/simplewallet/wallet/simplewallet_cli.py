...
DEFAULT_URL = 'http://172.22.0.3:8008'
...
def add_ipfs_parser(subparsers, parent_parser):
    '''Define the "ipfs-store" command line parsing.'''
    parser = subparsers.add_parser(
        'ipfs-store',
        help='transfers balance from one account to the other',
        parents=[parent_parser])
...
def create_parser(prog_name):
    ...
    add_ipfs_parser(subparsers, parent_parser)
...
def do_ipfs_store(args):
    ...
    response = client.ipfs_store(data, keyfile)
    print("Response: {}".format(response))
